...
//2.- Crear metodos (acciones que hara el controlador)
    ...
    const guardar = async(req, res)=>{
       /** */ try {
            ...
            if (!nombreProducto || !precio || !fechaCaducidad || !stock) {
                res.status(400).json({
                    status: "Error",
            ...
            if (!provedorEncontrado) {
                return res.status(404).json({
                    status: "Error",
                    message: "No se encontró el grupo musical con el ID proporcionado"
                });
            }
        }
    ...
    const SistemaPorId = async (req, res)=>{
        ...
        const productoBuscado= await Sistema.find({}).populate("provedor");
            ...
            if (!productoEliminado) {
                return res.status(404).json({
                    ...
                    message: "Producto no encontrado"
                        ...
                        if (productoEliminado.provedor) {
            await Provedor.findByIdAndDelete(productoEliminado.provedor, {
            $pull: { conciertos: productoEliminado._id }
            });
                }
    ...
    const SistemaActualizar = async(req, res)=>{
        try {
            ...
            const {nombreProducto, precio, fechaCaducidad, stock, imagen}= req.body;

            //3.- Validar que almenos un campo venga para actualizar
            if (!nombreProducto && !precio && !fechaCaducidad && !stock && !imagen ) {
            ...
            if (precio) datosActualizar.costo = precio;
            ...
            if (productoAcutualizado.grupo) {
            await Grupos.findByIdAndDelete(productoAcutualizado.grupo, {
            $pull: { conciertos: productoAcutualizado._id }
            });
                }
